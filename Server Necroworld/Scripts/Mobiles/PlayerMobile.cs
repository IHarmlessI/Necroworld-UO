...
namespace Server.Mobiles
{
	...
	public partial class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
		...
		public override int MaxWeight { get { return (((Core.ML && Race == Race.Human) ? 100 : 40) + (int)(5 * Str)); } }
		...
		public override int HitsMax
		{
			get
			{
				...
				if ((strBase)>=140)
					{ 
					return (strBase / 2) + 100 + strOffs;	
					}
				else
					{ 
					if ((strBase)>=130)
						{ 
						return (strBase / 2) + 90 + strOffs;
						}
					else
						{ 
						if ((strBase)>=120)
							{ 
							return (strBase / 2) + 80 + strOffs;
							}
						else
							{ 
							if ((strBase)>=110)
								{ 
								return (strBase / 2) + 70 + strOffs;
								}
							else
								{ 
								if ((strBase)>=100)

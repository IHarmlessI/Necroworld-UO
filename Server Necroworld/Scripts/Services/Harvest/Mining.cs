...
namespace Server.Engines.Harvest
{
    public class Mining : HarvestSystem
    {
        ...
        {
            if (tool is GargoylesPickaxe && def == OreAndStone && 0.5 > Utility.RandomDouble() && HarvestMap.CheckMapOnHarvest(from, harvested, def) == null)
            {
            ...
            if (tool is Shovel && def == OreAndStone && 0.1 > Utility.RandomDouble() && HarvestMap.CheckMapOnHarvest(from, harvested, def) == null)
            {
                HarvestResource res = vein.PrimaryResource;
